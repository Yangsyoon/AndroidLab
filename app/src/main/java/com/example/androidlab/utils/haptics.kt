...
object Haptics {
    ...
    fun vibrateEmotion(context: Context, emotion: String) {
        ...
        when (emotion) {
            ...
            // 😠 화남: 강하고 빠른 펄스 반복
            "화남" -> {
                pattern = longArrayOf(0, 150, 80, 150, 80, 150, 80)
                amplitudes = intArrayOf(0, 255, 0, 240, 0, 255, 0)
            }

            // 😊 기쁨: 점점 커지는 경쾌한 리듬
            "기쁨" -> {
                pattern = longArrayOf(0, 200, 100, 200, 100, 200, 100)
                amplitudes = intArrayOf(0, 170, 0, 190, 0, 210, 0)
            }

            // 😲 놀람: 짧고 강한 단발
            "놀람" -> {
                pattern = longArrayOf(0, 50, 150)
